...
import torch
import numpy as np
from tkg_qa_models.tcomplex import TComplEx
...
def loadTkbcModel_complex(tkbc_model_file):
    ...
    tcomplex_file = f'{data_root}/models/wikidata_big/kg_embeddings/tcomplex.ckpt'  # TODO: hack
...
def getAllDicts(dataset_name):
    ...
    if dataset_name == 'MultiTQ':
        base_path = f'{data_root}/data/{dataset_name}/kg/tkbc_processed_data'.format(dataset_name=dataset_name)
    ...
    base_path = f'{data_root}/data/{dataset_name}/kg/tkbc_processed_data/{dataset_name}/'.format(
    ...
    ts2id = dicts['ts_id']
    # hack
    file_ent = f'{data_root}/data/{dataset_name}/kg/wd_id2entity_text.txt'.format(
        dataset_name=dataset_name
    )
    file_rel = f'{data_root}/data/{dataset_name}/kg/wd_id2relation_text.txt'.format(
        dataset_name=dataset_name
    )
...
def checkIfTkbcEmbeddingsTrained(tkbc_model, dataset_name, split='test'):
    filename = f'{data_root}/data/{dataset_name}/questions/{split}.pickle'.format(
        dataset_name=dataset_name,
        split=split
    )
...
with open(f'{data_root}/saved_pkl/e2rt.pkl', 'rb') as f:
    e2rt = pickle.load(f)
with open(f'{data_root}/saved_pkl/event2time.pkl', 'rb') as f:
    event2time = pickle.load(f)
with open(f'{data_root}/saved_pkl/e2tr.pkl', 'rb') as f:
    e2tr = pickle.load(f)
