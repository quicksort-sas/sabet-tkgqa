...
def _worker_init(keys_list, keys_set):
    """Pool initializer: Each worker process loads the KG entity table once."""
...
def _fuzzy_match(text):
    """Find the closest matching KG entity name for a single entity text."""
...
if __name__ == "__main__":
    ...
    kg_path = f"{data_root}/data/MultiTQ/kg/tkbc_processed_data/ent_id"
    input_path = f"{data_root}/data/MultiTQ/questions/"
    output_path = f"{data_root}/data/MultiTQ/questions/processed_questions/"
    ...
    tagger = SequenceTagger.load(
        "flair/ner-english-large", cache_dir=f"{data_root}/.cache/flair"
    )
    ...
    for d in ["test", "dev", "train"]:
        ...
        # ---- 1) Flair NER batch prediction (GPU) ----
        questions = [Sentence(x["question"]) for x in dataset]
        for start in tqdm(
            range(0, len(questions), BATCH_SIZE), desc=f"  [{d}] NER", unit="batch"
        ):
            tagger.predict(
                questions[start : start + BATCH_SIZE], mini_batch_size=BATCH_SIZE
            )

        # ---- 2) Collect all NER outputs and deduplicate entity texts ----
        ...
        # ---- 3) Multiprocess difflib fuzzy matching ----
        text_to_match = {}
        if unique_texts:
            ctx = mp.get_context("fork")  # On Linux, fork shares read-only memory
            with ctx.Pool(
                processes=NUM_WORKERS,
                initializer=_worker_init,
                initargs=(keys, keys_set),
            ) as pool:
                chunksize = max(1, len(unique_texts) // (NUM_WORKERS * 8))
                for text, matched in tqdm(
                    pool.imap_unordered(
                        _fuzzy_match, unique_texts, chunksize=chunksize
                    ),
        ...
        # ---- 4) Write matching results back to the dataset ----
        for ix, spans in enumerate(tqdm(per_q_spans, desc=f"  [{d}] write", unit="q")):
            e = []
