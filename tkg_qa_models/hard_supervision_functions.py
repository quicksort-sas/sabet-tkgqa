...
def get_ents2reltime(kg_file):
    """
    Reads a tab-separated knowledge graph file and returns a dictionary mapping each (head, tail) entity pair to a set of full facts (including relation and time information).
    """
...
def get_event2time(kg_file):
    """
    Reads a tab-separated knowledge graph file and returns a dictionary mapping each event entity (head) to a set of facts only for a specific relation–entity pair (P793, Q1190554), including their associated time information.
    """
    with open(kg_file) as f:
        facts = f.read().strip().split('\n')
    facts = [f.strip().split('\t') for f in facts]
...
def retrieve_times(kg_file, dataset_name, data, corrupt_p, fuse):
    ...
    kg_file = f'{data_root}/data/{dataset_name}/kg/' + kg_file
